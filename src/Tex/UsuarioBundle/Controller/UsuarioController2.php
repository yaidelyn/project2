...
class UsuarioController extends Controller
{
    ...
    public function updateFormationAction(){
        if(!empty($_POST)){
            ...
            $formation->setEducation($_POST['education']);
            $formation->setStartDate(new \DateTime("$start_date[2]-$start_date[1]-$start_date[0]"));
            $formation->setEndDate(new \DateTime("$end_date[2]-$end_date[1]-$end_date[0]"));
            ...
            //actualizar File para CV
           // $update_formation = $em->getRepository('UsuarioBundle:Formation' )->find($formation->getId());

           // $update_formation->setFile();

            //$em->persist($update_formation);
            //$em->flush();
            ...
            $response = array("code" => 1, "success" => true,'mensaje'=>'Update!!!','id_formation'=>$id_formation);
            return new Response(json_encode($response));
        }
    }
    ...
    private function getPersonalsInfo($iduser = 0){
		...
		/*$pais =  $countries = $em->getRepository('UsuarioBundle:Paises')->findOneBy(array(
            'codigo'=>$profile[0]->getCountry()
        ));		

        $ciudad =  $countries = $em->getRepository('UsuarioBundle:Ciudades')->findOneBy(array(
            'paisesCodigo'=>$profile[0]->getCountry()
        ));
		
		$num = $profile[0]->getUser()->getCategory();*/
		  
        /*PROFESSIONAL CATEGORY*/
       // $category = $this->getNameCategoryUser($num);
       
		//$result['hcountry'] = $profile[0]->getCountry();
      ...
      //  $result["avatar"] = $profile[0]->getAvatar();
       ...
       // $result['birthday'] = $profile[0]->getBirthday();
		//$result['ciudad'] =  $profile[0]->getCity();
		//$result['latitud'] = $profile[0]->getUser()->getLatitud();
       // $result['longitud'] = $profile[0]->getUser()->getLongitud();
		//$result['category'] = array('id'=>$num,'category'=>$category);
        $result["profile_active"] = "active";
        $result["formation_active"] = "";
        $result["projects_active"] = "";
